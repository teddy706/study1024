...
export const ContactList: React.FC<Props> = ({ contacts }) => {
  ...
  const handleGenerateSmalltalks = async (e: React.MouseEvent, contactId: string, contactName: string) => {
    e.preventDefault() // Link 이동 방지
    e.stopPropagation()
    
    setGeneratingSmalltalks(prev => new Set(prev).add(contactId))
    
    try {
      const count = await smalltalkService.generateFromContactInfo(contactId)
      alert(`${contactName}님을 위한 스몰토크 ${count}개가 생성되었습니다! 상세 페이지에서 확인해보세요.`)
    } catch (error) {
      console.error('스몰토크 생성 실패:', error)
      alert('스몰토크 생성에 실패했습니다. 다시 시도해주세요.')
    } finally {
      setGeneratingSmalltalks(prev => {
  ...
  return (
    <div className="p-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div
          key={c.id}
          className="group relative bg-white rounded-2xl border border-gray-100 shadow-md shadow-gray-200/30 hover:shadow-xl hover:shadow-gray-300/40 hover:-translate-y-1 transition-all duration-300 overflow-hidden"
        >
          <Link
            to={`/contacts/${c.id}`}
            className="block p-5"
          ...
          <div className="px-5 pb-5 pt-2">
            <button
              onClick={(e) => handleGenerateSmalltalks(e, c.id, c.name || '고객')}
              disabled={generatingSmalltalks.has(c.id)}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {generatingSmalltalks.has(c.id) ? (
                <>
