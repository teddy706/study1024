...
🎯 CRITICAL RULE: If Personal Interests exist, generate EXACTLY 1 personal smalltalk that includes the exact interest keyword in the content text.
...
[
  ...
  ... (EXACTLY 3 items only - 정확히 3개만 생성)
]

🎯 ABSOLUTE REQUIREMENT - 절대 준수사항:
- Personal Interests가 존재하면: 3개 중 반드시 1개는 category="personal"이고 content에 관심사 키워드 직접 포함
- 관심사 키워드가 본문에 없으면 재생성 필요
- 예: 관심사="골프,와인" → 본문에 "골프" 또는 "와인" 단어 반드시 포함
...
🎯 CRITICAL: 정확히 3개의 스몰토크만 생성하세요. 3개 제한을 엄격히 준수하세요.

⚡ 우선순위 (3개 중 최소 1개는 개인 관심사 기반):
1. 개인 관심사 활용 스몰토크 (40-50%)
2. 회사/업무 관련 스몰토크 (40-50%)
3. 계절/업계 동향 스몰토크 (10-20%)

⚠️ 금지사항:
- ❌ 3개가 아닌 다른 개수의 스몰토크 생성 절대 금지
...
4. 개인 관심사 (50-70%, interests 있을 때 최우선 - 반드시 1개 생성):
...
핵심 원칙:
1. 🎯 무조건 정확히 3개의 스몰토크만 생성 (절대 다른 개수 금지)
2. 📋 3개 중 최소 1개는 개인 관심사(interests) 기반으로 생성
3. 모든 content는 최소 2문장, 50자 이상
4. 첫 문장은 구체적인 사실/정보 제공 (최근 3개월 이내)
5. 두 번째 문장은 자연스러운 질문 또는 제안 (향후 일정 중심)
6. 실제 대화에서 바로 사용 가능한 수준
7. 고객에게 가치있는 정보와 인사이트 제공
8. 시간 표현은 항상 구체적으로 (다음 주 월요일, 이번 달 말, 4분기 등)`
...
`
  ...
  // 관심사를 배열로 파싱하여 더 구체적으로 전달
  const interestsList = contact.interests 
    ? contact.interests.split(/[,\s]+/).filter(Boolean).map(i => i.trim())
    : []

  const interests = interestsList.length > 0
    ? `\n🎯 MANDATORY Personal Interests: ${interestsList.join(', ')}\n⚠️ CRITICAL: 3개 중 반드시 1개는 위 관심사 키워드를 직접 포함한 category="personal" 스몰토크 생성\n예시: 관심사가 "골프"면 본문에 "골프" 단어 필수 포함` 
    : '\n⚠️ 관심사 정보 없음: 회사/업무 중심으로 3개 생성'
